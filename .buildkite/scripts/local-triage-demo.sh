...
npx --no-install momentic install-browsers chromium
npx --no-install momentic run autoheal-test-authorship-demo.test.yaml \
  --output-dir "$RESULTS_DIR" \
  --upload-results

npx --no-install momentic ai triage "$RESULTS_DIR" \
  --yes \
  --quiet \
